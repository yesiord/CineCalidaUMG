...
CREATE TABLE promocionHasProducto (
	idPromocionHasProducto int not null primary key identity(1,1),
    idPromocion INT NOT NULL ,
    idProductoCine INT NOT NULL,
...
CREATE TABLE VentaHasProducto (
	idventaHasProducto int not null primary key identity(1,1),
...
alter table ventaHasProducto add idVentaHasProducto int not null primary key identity(1,1);
...
create table buzon(
idBuzon int not null primary key identity(1,1),
nombres varchar(75),
...
--  para stream de video dejarlo para final
...
alter table pelicula add idVideo int;
alter table pelicula add foreign key(idVideo) references video(idVideo);



select * from cineHasProducto inner join producto
on cineHasProducto.idproducto = producto.idproducto
inner join promocionHasProducto on
cineHasProducto.idCineHasProducto = promocionHasProducto.idcineHasProducto
inner join cine on
cine.idcine = cineHasProducto.idcine



go
alter procedure getProductosCine
	@pidCine int
AS
	select * from cine inner join cineHasProducto on
	cine.idcine = cineHasProducto.idcine inner join
	producto on producto.idproducto = cineHasProducto.idproducto
